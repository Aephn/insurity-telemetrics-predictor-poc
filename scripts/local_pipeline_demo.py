...
def generate_events(
    total: int, drivers: int, seed: int, extreme_variance: bool = False
) -> List[Dict[str, Any]]:
...
def ensure_model(model_dir: Path, force_retrain: bool = False) -> ModelArtifacts:
    ...
    artifacts, metrics = train_model(
        df, params=None, validation_size=0.2, early_stopping_rounds=10, num_boost_round=120
    )
...
def score(model: ModelArtifacts, feature_rows: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for m in missing:
        df[m] = 0
    # Ensure car_value_norm present; preserve raw
    if "car_value_norm" not in df.columns:
        if "car_value_raw" in df.columns:
            df["car_value_norm"] = df["car_value_raw"].astype(float) / 10000.0
        elif "car_value" in df.columns:
            if df["car_value"].max() > 500:  # raw dollars
                df.rename(columns={"car_value": "car_value_raw"}, inplace=True)
                df["car_value_norm"] = df["car_value_raw"].astype(float) / 10000.0
            else:  # already normalized
                df["car_value_norm"] = df["car_value"].astype(float)
                df["car_value_raw"] = (df["car_value_norm"] * 10000).round().astype(int)
                df.drop(columns=["car_value"], inplace=True)
    # Create interaction if absent
    if "car_speeding_interaction" not in df.columns:
        if {"car_sportiness", "speeding_minutes_per_100mi"}.issubset(df.columns):
            df["car_speeding_interaction"] = df["car_sportiness"].astype(float) * df[
                "speeding_minutes_per_100mi"
            ].astype(float)
        else:
            df["car_speeding_interaction"] = 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--model-artifacts", type=str, default="artifacts", help="Directory for model artifacts"
    )
    ...
    p.add_argument(
        "--inject-extremes",
        action="store_true",
        help="Add synthetic very-low and very-high risk feature rows for variance exploration",
    )
    p.add_argument(
        "--extreme-pairs",
        type=int,
        default=1,
        help="How many low/high extreme row pairs to inject (requires --inject-extremes)",
    )
    p.add_argument(
        "--extreme-variance",
        action="store_true",
        help="Enable generator risk profiles for wider raw event variance (data/mock.py)",
    )
    p.add_argument(
        "--force-retrain",
        action="store_true",
        help="Ignore existing model artifacts and retrain (useful after model sensitivity changes)",
    )
    p.add_argument(
        "--debug-driver-sample",
        type=int,
        default=5,
        help="Number of original (non-injected) driver rows to print with detailed feature + pricing breakdown",
    )
    p.add_argument(
        "--premium-target-spread",
        type=float,
        help="Override PREMIUM_SCALING_TARGET_SPREAD env for dynamic premium scaling (e.g. 0.45)",
    )
    p.add_argument(
        "--log-file", type=str, help="Custom log file path (default local_pipeline_demo.log)"
    )
    return p.parse_args()
...
def main() -> None:
    ...
    try:
        ...
        if p5 is not None and p95 is not None:
            spread = p95 - p5
            scaling_factor = (
                (float(tgt) if tgt != "default" else 0.35) / spread
                if spread and spread > 1e-6
                else float("nan")
            )
            log(
                f"Distribution stats: p5={p5:.4f}, p95={p95:.4f}, spread={spread:.4f}, target={tgt}, derived_scaling_factor≈{scaling_factor:.4f}"
            )
    ...
    pmin, pmax = float(priced_df["pricing.final_monthly_premium"].min()), float(
        priced_df["pricing.final_monthly_premium"].max()
    )
    ...
    if args.debug_driver_sample > 0:
        base_mask = (
            ~priced_df["driver_id"].str.startswith("DEXTR_")
            if "driver_id" in priced_df.columns
            else [True] * len(priced_df)
        )
